...
-- ManualAim, ThrowKnife, GrabGun: toggle only controls the enabled flag.
-- Their action buttons are always visible in the UI below.
...
local InvisibleToggle = MainTab:Toggle({
    Title    = "Invisible",
    Desc     = "Enable the Invisible button (teleports you 400 studs up for 5s)",
    Icon     = "eye-off",
    Type     = "Checkbox",
    Value    = false,
    Callback = function(state)
...
-- Shoot Murd button — always shown, errors if toggle off or conditions not met
local ShootMurdBtn = MainTab:Button({
    Title    = "Shoot Murd",
    Desc     = "Fire one aimed shot at the murderer",
    Icon     = "target",
    Callback = function()
        if not manualAimEnabled then
            WindUI:Notify({ Title = "Shoot Murd", Content = "Enable Manual Aim toggle first.", Duration = 3, Icon = "x" })
            return
        end
...
-- Throw Knife button — always shown, errors if not murderer or toggle off
local ThrowKnifeBtn = MainTab:Button({
    Title    = "Throw Knife",
    Desc     = "Throw knife at the nearest visible player",
    Icon     = "sword",
    Callback = function()
        if not throwKnifeEnabled then
            WindUI:Notify({ Title = "Throw Knife", Content = "Enable Throw Knife toggle first.", Duration = 3, Icon = "x" })
            return
        end
...
-- Grab Gun button — always shown, errors if toggle off or no gun dropped
local GrabGunBtn = MainTab:Button({
    Title    = "Grab Gun",
    Desc     = "Teleport to and grab the dropped gun",
    Icon     = "zap",
    Callback = function()
        if not grabGunEnabled then
            WindUI:Notify({ Title = "Grab Gun", Content = "Enable Grab Gun toggle first.", Duration = 3, Icon = "x" })
            return
        end
...
-- ── Invisible Button with animated 5s countdown ──────────────────────────────
--
-- State machine:
--   idle      → click → teleport up 400 studs → active (countdown 5→0)
--   active    → 5s elapsed → teleport back → idle
--
-- Animation: button title updates each second with a unicode fill bar.
-- Uses pcall on :Set() since WindUI may or may not support it on buttons.
-- Falls back to notify-only if :Set() is unavailable.

local INVIS_DURATION = 5
local INVIS_HEIGHT   = 400

local INVIS_BAR_FILL  = "█"
local INVIS_BAR_EMPTY = "░"

local function makeInvisBar(remaining, total)
    -- e.g. remaining=3, total=5 → "███░░"
    return string.rep(INVIS_BAR_FILL, remaining) .. string.rep(INVIS_BAR_EMPTY, total - remaining)
end

local InvisibleBtn

local function setInvisBtn(title, desc)
    -- try WindUI :Set() first, silently skip if unsupported
    if InvisibleBtn then
        pcall(function() InvisibleBtn:Set({ Title = title, Desc = desc }) end)
    end
end
...
local function resetInvisBtn()
    setInvisBtn(
        "Invisible",
        string.format("Vanish for %ds · teleports you %d studs above", INVIS_DURATION, INVIS_HEIGHT)
    )
end

InvisibleBtn = MainTab:Button({
    Title    = "Invisible",
    Desc     = string.format("Vanish for %ds · teleports you %d studs above", INVIS_DURATION, INVIS_HEIGHT),
    Icon     = "eye-off",
    Callback = function()
        ...
        if isInvisible then
            WindUI:Notify({ Title = "Invisible", Content = "Already active — wait for cooldown.", Duration = 2, Icon = "clock" })
            return
        end
        ...
        WindUI:Notify({
            Title    = "Invisible",
            Content  = string.format("Vanished! Returning in %ds...", INVIS_DURATION),
        ...
        task.spawn(function()
            for i = INVIS_DURATION, 1, -1 do
                setInvisBtn(
                    string.format("Invisible  [%ds]  %s", i, makeInvisBar(i, INVIS_DURATION)),
                    string.format("Returning in %d second%s...", i, i ~= 1 and "s" or "")
                )
                task.wait(1)
            end
...
myConfig:Register("Invisible",    InvisibleToggle)
...
lp.CharacterAdded:Connect(function(char)
    ...
    watchLpGun(char)
    refreshLpMurd()
    -- if invisible was active when character respawned, cancel it cleanly
    if isInvisible then
        isInvisible   = false
        invisReturnCF = nil
        resetInvisBtn()
    ...
    if hum then
        hum.Died:Connect(function()
            if playersInRound[lp] then playersInRound[lp] = nil end
            -- cancel invisible on death so we don't return to a dead position
            if isInvisible then
                isInvisible   = false
                invisReturnCF = nil
                resetInvisBtn()
...
WindUI:Popup({
    Title   = "MM2 ShadowX V2.120.11",
    Icon    = "sword",
    Content = "V2.120.11 — Persistent action buttons, Invisible feature, button feedback.",
    Buttons = {
        {
            Title    = "Close",
